...
app.use("/upload", (request, response) => {
    new Promise((resolve) => {
        ...
        if (!name || !name[0] || request.method !== 'POST') {
            throw new HTTPException(409, "Only POST requests allowed");
        }
                                                // jeśli nie, odrzucamy ze statusem 409 Conflict, inne kody np. tutaj:
    ...
    .then(
        ...
        .then(
            ...
            () => thenify(fs.mkdir)(path.dirname(target))
        ...
        .catch(
            (e) => {
                ...
                throw new HTTPException(500);       // i wysyłamy status błędu do klienta
                                                    // (to konieczne, inaczej klient się nie rozłączy!)
            }
        )
    )
    .catch(
        e => response.sendStatus(e.status, e.isClientException ? e.message : "Internal server error")
    )
    .catch(
